...
1. Read `references/composition-guidelines.md` and `references/music-complexity.md`.
...
8. Turn the rhythm into a short core melodic motif.
9. Write functional harmony and instrument-appropriate voicings.
10. Test the motif against the harmony before expanding the arrangement.
11. Write section melodies using repetition, variation, contrast, and return.
12. Write bass with harmonic and rhythmic direction, not roots alone.
13. Write drums with section-specific groove and transitions.
14. Write piano and guitar as distinct playable parts.
15. Write strings and pad as support, counter-motion, atmosphere, or climax—not chord-block duplicates.
16. Apply restrained MIDI cleanup/humanization: purposeful velocity, duration, articulation, and timing variation while preserving anchors.
17. If vocals were explicitly requested, follow the optional vocal workflow below; otherwise skip it completely.
18. Save the initial draft as `composition_v1.json`, copy it to `composition.json`, and render it.
19. Run `scripts/critic_complexity.py <song> --write`, then listen when possible and analyze stems/mix for silence, duration, clipping, section energy, density, and contrast.
20. Complete `references/composer-checklist.md` and write concrete findings to `projects/<song>/critique.md`.
21. Make at least one composition revision. Save it as `composition_v2.json` (and later numbered versions); never erase the only prior version.
22. Copy the selected revision to `composition.json` and render the final candidate.

...
def _operations(base: list[dict[str, Any]], relationship: dict[str, Any], phrase_index: int) -> list[dict[str, Any]]:
    ...
    if "compression" in operations:
        for item in result:
            item["offset"] *= 0.75; item["duration"] *= 0.75
    if "augmentation" in operations:
        for item in result:
            item["offset"] *= 1.25; item["duration"] *= 1.25
    transpose = 0
...
def compile_long_form_lead(phrase: dict[str, Any], beats_per_bar: int,
    ...
    scale_pcs = {root_pc(phrase.get("key_root", "E")) + step for step in (0, 2, 3, 5, 7, 9, 10)}
    scale_pcs = {value % 12 for value in scale_pcs}
    ...
    for phrase_index, relation in enumerate(relationships):
        ...
        for note_index, item in enumerate(transformed):
            ...
            if relation["relationship"] == "climax" and note_index == len(transformed) - 1:
                # Reserve the target itself for the planned late peak rather than
                # letting a two-bar motif finish at the front of a four-bar node.
            ...
            function = "chord_tone" if pitch % 12 in {chord_pc, (chord_pc + 3) % 12, (chord_pc + 4) % 12, (chord_pc + 7) % 12} else "non_chord_tone"
            ...
            if action in {"hammer_on", "pull_off"}: arts.extend([action, "legato"])
            elif action == "slide": arts.extend(["slide", "legato"])
            elif action == "bend" and allow_pitch_bend: arts.append("bend")
            is_final = relation["relationship"] == "resolution" and note_index == len(transformed) - 1
            if is_final: arts.append("resolution")
            ...
            event = note(pitch, start, duration, velocity, beats_per_bar, arts,
            ...
            state["direction"] = "ascending" if pitch > previous_pitch else ("descending" if pitch < previous_pitch else "level")
        ...
        state_trace.append({"phrase_id": relation["phrase_id"], "point": "end", **deepcopy(state),
                            "rest_type": "structural_end" if state["resolved"] else "breath"})

    plan = {
        "schema_version": 1, "section_arc": deepcopy(arc),
        "phrase_relationships": deepcopy(relationships), "melodic_state_trace": state_trace,
        "long_form_phrase_rules": deepcopy(rules),
    }
    events = sorted(events, key=lambda event: position(event["at"], beats_per_bar))
    # Safe default for a single-channel lead: one note ends before the next begins.
    if not phrase.get("realization", {}).get("allow_profile_legato_overlap", False):
        for current, following in zip(events, events[1:]):
            ...
            current["duration"] = round(max(0.05, min(float(current["duration"]), next_start - current_start)), 3)
    return events, plan
